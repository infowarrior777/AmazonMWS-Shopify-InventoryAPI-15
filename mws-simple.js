...
Client.prototype.request = function(requestData, callback) {
  ...
  request.post(options, function (error, response, body) {
    ...
    if (response.headers.hasOwnProperty('content-type') && response.headers['content-type'].startsWith('text/xml')) {
      // xml2js
            xmlParser(body,{ explicitArray : false, ignoreAttrs : true, mergeAttrs: true }, function (err, result) { // added param to xml parser to get a normal object result { explicitArray : false, ignoreAttrs : true, mergeAttrs: true },
        callback(err, result);
      });
    } else {
